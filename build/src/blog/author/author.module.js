"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
var common_1 = require("@nestjs/common");
var author_service_1 = require("./author.service");
var author_resolver_1 = require("./author.resolver");
var user_module_1 = require("../../../../../../../../../../src/user/user.module");
var user_service_1 = require("../../../../../../../../../../src/user/user.service");
var user_resolver_1 = require("../../../../../../../../../../src/user/resolver/user.resolver");
var AuthorModule = /** @class */ (function () {
    function AuthorModule() {
    }
    AuthorModule = __decorate([
        common_1.Module({
            imports: [user_module_1.UserModule],
            providers: [author_service_1.AuthorService, author_resolver_1.AuthorResolver, user_service_1.UserService, user_resolver_1.UserResolver],
        })
    ], AuthorModule);
    return AuthorModule;
}());
exports.AuthorModule = AuthorModule;
//# sourceMappingURL=author.module.js.map